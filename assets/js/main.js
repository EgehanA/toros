 ...
 */
  ...
  *  Veri Silme
  */
  jQuery(document).on('click', '[data-delete]', function(event) {
     ...
     .then((willDelete) => {
       if (willDelete) {
         swal({
           text: 'İstek gerçekleştiriliyor... Lütfen Bekleyiniz!',
           content: "text",
           icon: 'info',
           button: false
         });
         $.ajax({
             ...
             dataType: 'json',
             cache: false,
							data: {id: $(el).attr('data-id'), table: $(el).attr('data-table'), type:'delete'}
         })
         .done(function(result) {
             ...
             if(result.s == 'success')
             {
                 $(el).parent('td').parent('tr').fadeOut();
             }
         })
         .fail(function(){
             swal({
                 title:  'Fatal Error!',
                 text:   'Sistem hatası oluştu, teknik destek alın!',
                 icon:    'error',
                 buttons:    ['Tamam']
             });
         });
